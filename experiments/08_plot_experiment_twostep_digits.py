...
# load data
EXPE_NAME = "twostep_digits"
results = torch.load(RESULTS_FOLDER / f"annealed_nce_expe_{EXPE_NAME}.th")
df = pd.DataFrame(results)

# reformat dataframe
df = df.replace(
    to_replace="arithmetic-adaptive", value="(oracle)")
df = df.replace(
    to_replace="arithmetic-adaptive-trig", value="(oracle-trig)")
sel_replace = (df.n_distributions == 2) & (df.path_name == "(oracle-trig)")
...
estimates = np.concatenate(
    [df.iloc[row].logZs for row in range(len(df))]).flatten()
...
sns.violinplot(
    ...
    ax=ax,
    order=["no path", "(oracle)", "(oracle-trig)"],
    inner=None,
    # showmeans=True,
    # meanprops={"marker": "o",
